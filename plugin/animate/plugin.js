...
const initAnimate = function (Reveal) {
  ...
  function getAnimatedSVG(container) {
    ...
    var svg = elements
      .toArray()
      .find((element) => element.node.parentElement == container);
        ...
        try {
          if (setup[i].element) {
            ...
            if (typeof setup[i].modifier === "function") {
              ...
              container.svg[setup[i].modifier].apply(
                container.svg,
                setup[i].parameters
              );
            }
  ...
  function seek(timestamp) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (elements[i].animation && elements[i].animationSchedule[fragment]) {
        elements[i].animation.time(
          elements[i].animationSchedule[fragment].begin + (timestamp || 0)
        );
      }
    }
    if (timer) {
  ...
  function createPrintout() {
    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      for (var j = 0; j < elements.length; j++) {
        ...
        if (
          elements[j].animation &&
          elements[j].animationSchedule &&
          elements[j].animationSchedule[fragment]
        ) {
          //console.log(i,fragment, elements[j].animationSchedule[fragment].begin);
          elements[j].animation.time(
            elements[j].animationSchedule[fragment].end
          );
        }
